...
export default function EditarAluno() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/api/alunos/${id}/`)
      ...
      .then((data) => {
        ...
        setTurmaId(data.turma || data.turma_id || "");
        if (data.foto_aluno) {
          setPreview(data.foto_aluno.startsWith('http') ? data.foto_aluno : `http://localhost:8000${data.foto_aluno}`);
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const matriculaExiste = alunosData.alunos.some(
      (a) => a.matricula_aluno === matricula && a.id !== Number(id)
    );
    if (matriculaExiste) {
    ...
    // Envio
    const res = await fetch(`http://localhost:8000/api/alunos/${id}/update/`, {
      method: "PUT",
    ...
    if (res.ok) {
      setSucesso(true);
      // Limpa apenas campos do formulário principal, não mexe na foto
    } else {
      let data;
      try {
        data = await res.json();
      ...
      setErro(data.error || "Erro ao atualizar aluno");
    }
  };
  ...
  const handleFotoSubmit = async (e) => {
    ...
    formData.append("foto_aluno", foto);
    const res = await fetch(`http://localhost:8000/api/alunos/${id}/foto/`, {
      method: "PATCH",
      body: formData,
    });
    if (res.ok) {
      setSucesso(true);
      // Não limpa preview/foto para manter UX consistente
    } else {
      let data;
      try {
        data = await res.json();
      ...
      setErro(data.error || "Erro ao atualizar foto do aluno");
    }
  };

  return (
    <div className="flex flex-col items-center justify-center-up min-h-[80vh] mt-5">
      <h1 className="text-2xl font-bold mb-4 drop-shadow-[0_1px_0_rgba(0,0,0,0.8)]">Editar Aluno</h1>
      ...
      {sucesso && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="relative bg-white rounded-xl shadow-lg p-6 max-w-xs w-full flex flex-col items-center">
            ...
            <p className="text-gray-800 text-center mb-4">Aluno atualizado com sucesso!</p>
            <button
